...
RetroArch 1.22.2 and one custom core: **mGBA Multi 0.2.0**. It builds on the
...
- Player 1 runs on RetroArch's libretro caller thread. Players 2 and 3 use
  ...
  **Parallel**. No frame queue or run-ahead is permitted. At normal speed, one
  `retro_run` call advances every active instance by exactly one frame. Speed-up
  loops a bounded one to four frames on the same owner lane and within the same
  worker generation; it never dispatches extra jobs or leaves queued work.
...
audio-cadence, rate-transition, and RTC-container tests.
...
The selected ROM is cloned into each active instance. Version 0.2.0 does not
...
The validator requires the audio selector, all three speed-target options, and
the R2 speed descriptor. It rejects a stock mGBA framework, mGBA's
